 ...
 */
export function schemaToDSL(schemaPath) {
    try {
        ...
        for (const col of schema.columns) {
            ...
            if (col.defaultFn)
                constraints.push(`DEFAULT ${col.defaultFn}()`);
            else if (col.default)
                constraints.push(`DEFAULT ${col.default}`);
            if (col.enum)
                constraints.push(`ENUM(${col.enum.join(',')})`);
            if (col.references) {
 ...
 */
export function getAllSchemasDSL(schemaDir = 'config/schema') {
    ...
    const dslParts = ['// === EXISTING DATABASE SCHEMAS ===', ''];
 ...
 */
export function apiToDSL(apiPath) {
    try {
 ...
 */
export function getAllAPIsDSL(apiDir = 'config/api') {
    ...
    const dslParts = ['// === EXISTING API ENDPOINTS ===', ''];
    for (const file of files) {
 ...
 */
export function pageToDSL(pagePath) {
    try {
        ...
        if (page.children && page.children.length > 0) {
            ...
            const componentTypes = page.children.map((c) => c.type);
 ...
 */
export function getAllPagesDSL(pagesDir = 'config/pages') {
 ...
 */
export function getAPIContext(resourceName, isRegenerate = false) {
    const parts = [];
 ...
 */
function extractAPIRequestResponse(apiPath) {
    try {
        const content = readFileSync(apiPath, 'utf-8');
        const lines = [];
        const resourceName = apiPath.split('/').pop()?.replace('.routes.ts', '') || 'unknown';
 ...
 */
export function buildContext(type, options = {}) {
    switch (type) {
        case 'schema':
...
//# sourceMappingURL=dsl-converters.js.map