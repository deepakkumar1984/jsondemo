...
async function generateConfig(
  ...
  // Warn if description is complex
  const entityCount = description.split(',').length;
  if (entityCount > 2 && (type === 'api' || type === 'pages')) {
    console.log(`\n⚠️  Generating for ${entityCount} entities may take longer and risk timeout.`);
    console.log(`   Consider generating one at a time for faster results.\n`);
  }
  ...
  try {
    ...
    // Call AI worker with extended timeout
    console.log(`\n📡 Calling AI worker...`);
    ...
    if (error.name === 'AbortError' || error.message.includes('aborted')) {
      console.error('\n💡 Request took longer than 3 minutes. Try:');
      console.error('  1. Generate one entity at a time (e.g., "tasks" instead of multiple entities)');
      console.error('  2. Use simpler descriptions');
      console.error('  3. Check if AI worker is responding: curl http://localhost:8787');
    } else if (error.message.includes('timeout') || error.message.includes('504')) {
      console.error('\n💡 Cloudflare gateway timeout (max ~100 seconds). Try:');
      console.error('  1. Generate ONE entity at a time (e.g., "tasks" not "tasks, projects, users")');
      console.error('  2. Use concise descriptions without excessive details');
      console.error('  3. For API generation, generate for single table');
      console.error('  4. Wait a moment and retry - sometimes the AI model is overloaded');
    } else if (error.message.includes('fetch')) {
      console.error('\n💡 Cannot connect to AI worker. Make sure it is running:');
      console.error('  npm run ai:dev');
      console.error('  (Should be running on http://localhost:8787)');
    } else {
